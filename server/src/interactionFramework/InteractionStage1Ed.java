 ...
 *
 */
public class InteractionStage1Ed {
	...
	//private static String problemFile = "DUTPtoyexampleDC.xml";        Ed's
...
//				String resp2 = cin.next();
				...
				if(numAgents > 1 && currentAgent != numAgents)
					System.out.println("Select activity for agent " + currentAgent+" to perform: " + activities.get(0).toString() + " or (O)utput schedule information, (U)ndo last selection, (C)hange agent, or (A)dvanced selection.");
				else if(numAgents > 1){
...
//					System.out.println("System time is: " + getSystemTime());
	...
	private static double getMaxLatestEndTime(DisjunctiveTemporalProblem dtp) {
		...
		for (Interval interv : minmaxArray) {
			// Drew: Max seems more appropriate here. Otherwise this ends early
	...
	private static void processSporadicEvent(String last_act) {
		if(last_act.length() == 0){
			...
			System.out.println("Sporadic event couldn't have happened since last check in time.");
			return;
		}
		...
		if(resp.equalsIgnoreCase("y")){
			...
			System.out.println("At what time did the sporadic event happen? ");
			String time = cin.next();
			if(true) {
				String sename = DUTP.SPORADICNAME;
				int se_time = Generics.fromTimeFormat(time);
				System.out.println("Inserting sporadic activity at end of previous activity, for duration " + DUTP.DUR);
			...
			System.out.println("Inserting sporadic event in beginning at " + time + "\n" );
...
/**
		...
		**/
			...
			if (requiredActivity(last_activity)) {
				System.out.println("No sporadic and a required activity.");
				dtp.advanceDownTree();
...
//				String resp1 = cin.next();
	...
	private static void getAndPerformIdle(String str, int minIdle, int maxIdle){
		...
		System.out.println("How long to idle? Possible Options {["+Generics.toTimeFormat(minIdle)+", "+Generics.toTimeFormat(maxIdle)+"]}");
